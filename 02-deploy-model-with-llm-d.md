...
### 8.3 Validate Your Model with cURL

Once the model is deployed and the endpoint with HTTPRoutes is created, you can validate that it responds correctly using `curl`. Make sure you have the endpoint and appropriate token.
...
Then execute this command to test the model with chat completions:

```bash
curl -k -X POST ${MODEL_ENDPOINT}/v1/chat/completions \
...
- **Error 403 (Forbidden)**: The token doesn't have permissions. Make sure to use `oc whoami -t` to get your user token.
- **jq error "Invalid numeric literal"**: The response is not valid JSON (may be HTML error). Try without `jq` first to see the actual response:
  ```bash
  curl -k -X POST ${MODEL_ENDPOINT}/v1/chat/completions \
    -H "Authorization: Bearer ${TOKEN}" \
    -H "Content-Type: application/json" \
    -d '{"model": "'${SERVICE_NAME}'", "messages": [{"role": "user", "content": "Hello, world!"}]}'
  ```

If the error persists, try a GET request first to verify the connection:

```bash
# Verify that the endpoint responds correctly
curl -k -H "Authorization: Bearer ${TOKEN}" ${MODEL_ENDPOINT}/v1/models | jq .
```

Or if you prefer to test with completions (text completion):

```bash
curl -k -X POST ${MODEL_ENDPOINT}/v1/completions \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "'${SERVICE_NAME}'",
...
> 💡 **Notes:**
> - If your model has a different name, adjust the `SERVICE_NAME` variable or the `"model"` field in the JSON payload.
> - If your deployment doesn't require authentication, you can omit the `Authorization` header.
> - Using `jq` is optional but helps improve the readability of the response.

If everything is correct, you should receive a response with the output generated by your model.
